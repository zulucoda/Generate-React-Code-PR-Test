...
export function exampleAsyncAction() {
    return (dispatch, getState) => {
        const { candyApp2Reducer } = getState();
        dispatch(exampleAction(!candyApp2Reducer.exampleVariable));
    };
}
...
export default function candyApp2Reducer(state = initialSate, action) {
    switch (action.type) {
