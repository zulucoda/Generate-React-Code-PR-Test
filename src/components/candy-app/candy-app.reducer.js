...
export function exampleAsyncAction() {
    return (dispatch, getState) => {
        const { candyAppReducer } = getState();
        dispatch(exampleAction(!candyAppReducer.exampleVariable));
    };
}
...
export default function candyAppReducer(state = initialSate, action) {
    switch (action.type) {
